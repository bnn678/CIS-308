...
int GetProgramState()
{
	...
	printf("Enter [i]nsert, [s]earch, inorder [t]ransversal, [r]emove, or [q]uit: ");

	while (true)
	{
		state = fgetc(stdin);

		if (state == 'q' || state == 'i' || state == 's' || state == 't' || state == 'r')
		{
			switch (state)
			{
			case ('q'):
...
* Precondition: the user enters an int that no more than 5 digits
...
/***********************************************************************
* Description: Get the user's desired number to remove in the BST
* Precondition: the user enters an int that no more than 5 digits
* Postcondition: the function returns the int
***********************************************************************/
int GetRemoveNum()
